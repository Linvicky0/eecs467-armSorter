...
2. If you haven't built befpre, you will have to run this command "cd ~/ros2_ws
colcon build --symlink-install --packages-select realsense2_camera --cmake-args -DCMAKE_BUILD_TYPE=Release"
3. So far I have ot updated the launch files, so running this command will boot up the camera to start publishing the topics
"source ~/ros2_ws/install/setup.bash
ros2 launch realsense2_camera rs_launch.py device_type:=d435i align_depth.enable:=true"
5. You should get something like this in the terminal "[realsense2_camera_node-1] [INFO] [1775329998.089389998] [camera.camera]: Open profile: stream_type: Color(0), Format: RGB8, Width: 1280, Height: 720, FPS: 30
